    ...
    if Config.FORCE_SUB:
        try:
            ...
            if chat.status=='kicked':
                return await m.reply_text("You have been kicked from our updates channel. So, you can't use me",  quote=True)
            ...
            logger.warning(f"Make me an admin in @{Config.FORCE_SUB}")
            if m.from_user.id in Config.AUTH_USERS:
                return await m.reply_text(f"Make me an admin in @{Config.FORCE_SUB}")
            ...
            logger.error(e)
            return await m.reply_text("Some thing went wrong. Try again and if same issue occur contact [our group](https://t.me/RedX14)", disable_web_page_preview=True, quote=True)
